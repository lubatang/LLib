...
bool LoadTGA(Texture * texture, const char* filename)        // Load a TGA file
{
  ...
  {    
    fprintf(stderr, "Could not open texture file: %s\n", filename);  // Display an error message
    return false;                            // Exit function
  }

  if(fread(&tgaheader, sizeof(TGAHeader), 1, fTGA) == 0)          // Attempt to read 12 byte header from file
  {
    fprintf(stderr, "Could not read file header: %s\n", filename);    // If it fails, display an error message 
  ...
  {
    fprintf(stderr, "TGA file %s be type 2 or type 10.\n", filename);
    fclose(fTGA);
    return false;                                // Exit function
  }
...
{                                      // TGA Loading code nehe.gamedev.net)
  ...
  {                    
    fprintf(stderr, "Could not read into header!\n");
  ...
  {
    fprintf(stderr, "Invalid texture information in %s", filename);
  ...
  {
    fprintf(stderr, "Could not allocate memory for image %s.\n", filename);  // Display Error
...
{ 
  ...
  do
  {
    ...
    {
      fprintf(stderr, "Could not read RLE header of %s!\n", filename);  // Display Error
      if(fTGA != NULL)                        // If file is open
      {
        fclose(fTGA);                        // Close file
      }
    ...
    {                                  // that follow the header
      ...
      for(short counter = 0; counter < chunkheader; counter++)    // Read RAW color values
      {
        ...
        {
          fprintf(stderr, "Could not read image data: %s!\n", filename);    // IF we cant, display an error
    ...
    else                                      // chunkheader > 128 RLE data, next color reapeated chunkheader - 127 times
    {
      ...
      {  
        fprintf(stderr, "Could not read from file %s!\n", filename);      // If attempt fails.. Display error (again)
